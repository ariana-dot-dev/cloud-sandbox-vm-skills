...
- **Disposable & cheap**: `boat delete` when done; per-second billing, a run costs cents.
...
trap 'boat delete "$sandbox_id" --json' EXIT          # never leak a VM
boat ssh "$sandbox_id" -- "mkdir -p /run && cat > /run/main.sh" < ./main.sh
boat ssh "$sandbox_id" -- "cd /run && timeout 120 bash main.sh"
```
