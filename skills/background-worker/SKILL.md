...
# Background / 24-7 worker (Boat)

Use this when work must **keep running unattended**: a queue worker, a Telegram/Discord bot, a polling agent, a scheduled scraper, or any always-on service. Ephemeral sandboxes (E2B microVMs, Codespaces) auto-die on a timer: wrong tool. A Boat sandbox can be told to **never auto-stop**, persists its disk, and keeps a dedicated IPv4.

## Why Boat

- **No session cap**: runs hours/days/indefinitely.
- **Persistent + resumable**: disk survives stop; `boat resume` brings it back.
...
boat ssh "$sandbox_id" -- "cd /app && nohup ./worker.sh > /var/log/worker.log 2>&1 &"
boat extend "$sandbox_id" --ttl 2592000                 # optional: 30-day deadline
```
