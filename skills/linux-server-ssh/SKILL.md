...
- **First-class SSH**: `boat ssh <id>` opens a session; the CLI handles keys. SCP works too.
...
boat ssh "$sandbox_id"                                  # interactive shell
boat ssh "$sandbox_id" -- "sudo apt-get install -y nginx && sudo systemctl enable --now nginx"
boat info "$sandbox_id" --json                          # shows the IP
```
